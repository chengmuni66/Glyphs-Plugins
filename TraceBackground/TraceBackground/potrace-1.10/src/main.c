...
// { "svg",        ".svg", 0, 0, 0,   NULL,     page_svg,     NULL,     1 },
			{ "Glyphs",        ".glyphsPath", 0, 0, 0,   NULL,     page_Glyphs,     NULL,     1 },
...
static void dopts(int ac, char *av[]) {
	...
	while ((c = getopt_long(ac, av, shortopts, longopts, NULL)) != -1) {
		//fprintf(stdout, "// __c: %c\n", c);
		switch (c) {
			case 'h':
				fprintf(stdout, "potrace %s. Transforms bitmaps into vector graphics.\n\n", VERSION);
				//usage(stdout);
				exit(0);
				break;
			case 'v':
			case 'V':
				fprintf(stdout, "potrace %s. Copyright (C) 2001-2011 Peter Selinger.\n", VERSION);
...
//				break;
	...
	fprintf(stderr, "potrace: input file -- %s\n", info.infiles[0] );
...
*/
	fprintf(stderr, "potrace: info.infilecount %d info.infiles[0]: %s\n", info.infilecount, info.infiles[0]);
	
	if (!info.some_infiles) {                 /* read from stdin */
		fprintf(stderr, "// potrace: __1\n");
		...
		fprintf(stderr, "// potrace: < __2\n");
		//for (i=0; i< info.infilecount; i++) {
		i = 0;
		fin = my_fopen_read(info.infiles[i]);
		if (!fin) {
			fprintf(stderr, "potrace: input: %s: %s\n", info.infiles[i], strerror(errno));
			exit(2);
		}
		fout = stdout; // my_fopen_write(outfile);
		fprintf(stderr, "potrace: < process \n");
