...
/* calculate p1 x p2 */
static inline int xprod(point_t p1, point_t p2) { return p1.x * p2.y - p1.y * p2.x; }
...
static inline double ddist(dpoint_t p, dpoint_t q) { return sqrt(sq(p.x - q.x) + sq(p.y - q.y)); }
...
static int opti_penalty(privpath_t *pp, int i, int j, opti_t *res, double opttolerance, int *convc,
						double *areac) {
	int m = pp->curve.n;
	int k, k1, k2, conv, i1;
...
static int opticurve(privpath_t *pp, double opttolerance) {
	...
	for (i = om - 1; i >= 0; i--) {
		if (pt[j] == j - 1) {
			...
			pp->ocurve.c[i][2] = pp->curve.c[mod(j, m)][2];
			pp->ocurve.vertex[i] =
				interval(opt[j].s, pp->curve.c[mod(j, m)][2], pp->curve.vertex[mod(j, m)]);
...
int process_path(path_t *plist, const potrace_param_t *param, progress_t *progress) {
	...
	if (progress->callback) {
		...
		nn = 0;
		list_forall(p, plist) { nn += p->priv->len; }
		cn = 0;
	}
