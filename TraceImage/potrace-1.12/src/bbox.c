...
static inline void singleton(interval_t *i, double x) { interval(i, x, x); }
...
static inline int in_interval(interval_t *i, double x) { return i->min <= x && x <= i->max; }
...
static double iprod(dpoint_t a, dpoint_t b) { return a.x * b.x + a.y * b.y; }
...
void path_limits(potrace_path_t *path, dpoint_t dir, interval_t *i) {
	...
	/* iterate */
	list_forall(p, path) { curve_limits(&p->curve, dir, i); }
}
