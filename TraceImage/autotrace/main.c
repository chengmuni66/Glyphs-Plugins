...
static char *read_command_line(int, char *[], at_fitting_opts_type *, at_output_opts_type *);
...
static char *read_command_line(int argc, char *argv[], at_fitting_opts_type *fitting_opts,
	...
	int option_index;
	struct option long_options[] = {{"align-threshold", 1, 0, 0},
	...
	while (true) {
		...
		if (ARGUMENT_IS("background-color")) {
			...
			fitting_opts->background_color =
				at_color_new((unsigned char)(hctoi(optarg[0]) * 16 + hctoi(optarg[1])),
